...
def _get_signing_key() -> SigningKey:
    if not PRIVATE_KEY_B64:
        raise RuntimeError("BECKN_PRIVATE_KEY not set in .env — external BPP signing unavailable")
    return SigningKey(base64.b64decode(PRIVATE_KEY_B64))
...
def make_auth_header(body: dict, expires_in: int = 300) -> str:
    ...
    Returns a no-op header for local dev when no key is configured.
    """
    if not PRIVATE_KEY_B64:
        return f'Signature keyId="{SUBSCRIBER_ID}|local-dev|ed25519",algorithm="ed25519",signature="local-dev-no-sig"'
    ...
    signed    = _get_signing_key().sign(signing_string.encode())
