...
async def create_filing(req: FilingRequest):
    ...
    # Build all 4 Beckn action payloads
    action_bodies = {
        "select": {
            "context": beckn.make_context("select", txn_id),
            "message": {"contract": _build_contract(filings, "DRAFT", "DRAFT")},
        },
        "init": {
            "context": beckn.make_context("init", txn_id),
            "message": {"contract": _build_contract(filings, "ACTIVE", "ACTIVE")},
        },
        "confirm": {
            "context": beckn.make_context("confirm", txn_id),
            "message": {"contract": _build_contract(filings, "ACTIVE", "ACTIVE",
                                                     with_settlement=True)},
        },
        "status": {
            "context": beckn.make_context("status", txn_id),
            "message": {"contract": {
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for action, body in action_bodies.items():
            ...
            # 1. Log through gateway for Beckn tracking
            try:
                await client.post(f"{beckn.BECKN_GW}/{action}", json=body)
            except Exception:
                pass

            # 2. Call our RDE BPP directly — gets real receipt back
            try:
                r = await client.post(f"{OUR_BPP}/{action}/rde", json=body)
                bpp_responses[action] = r.json()
                steps.append(_step(action, bpp_responses[action]))
            except Exception as e:
                bpp_responses[action] = {"_error": str(e)}
                steps.append(_step(action, bpp_responses[action]))

    # ── Extract receipt from confirm (carries hash validation + status) ────────
    receipt = None
    for action in ["confirm", "status"]:
        contract = bpp_responses.get(action, {}).get("message", {}).get("contract", {})
        for perf in contract.get("performance", []):
            candidate = perf.get("performanceAttributes")
            if candidate and candidate.get("receiptStatus"):
                receipt = candidate
                break
        if receipt:
            break
    ...
    return {
        ...
        "status":        receipt.get("receiptStatus", "UNKNOWN") if receipt else "NO_RECEIPT",
        "filingId":      receipt.get("filingId")    if receipt else None,
