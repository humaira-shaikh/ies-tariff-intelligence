...
async def who_is_accessing():
    ...
    try:
        ...
        # ngrok not running locally — return local connection info
        return {
            "total_unique_visitors": 1,
            "our_ngrok_url": "http://localhost:9000 (local)",
            "visitors": [{
                "ip":             "127.0.0.1",
                "team":           "localhost (you)",
                "total_requests": len(beckn._responses),
                "paths_visited":  ["/bpp/receiver/select", "/bpp/receiver/confirm",
                                   "/bpp/receiver/status"],
            }],
        }
