...
const TEAMS  = {
  'bpp.example.com':     'http://127.0.0.1:9000/bpp/receiver',
...
export default function TIPanel({ initialSection = 'flow', onBack }) {
  const [section, setSection] = useState(initialSection)
  const [sharedPolicies, setSharedPolicies] = useState([])

  return (
    <div>
      {/* Back to dashboard + Sub-nav */}
      <div style={{ display:'flex', alignItems:'center', gap:'1rem', marginBottom:'1.25rem', flexWrap:'wrap' }}>
        {onBack && (
          <button onClick={onBack} style={{
            padding:'0.35rem 0.875rem', borderRadius:'8px', border:'1.5px solid #E2E8F0',
        ...
        <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap' }}>
          {[
            ...
            <button key={id} onClick={() => setSection(id)} style={{
              ...
              border: `1.5px solid ${section===id ? '#1652F0' : '#E2E8F0'}`,
              cursor:'pointer', fontSize:'0.8rem', fontWeight:600,
              background: section===id ? '#1652F0' : '#fff',
...
function PolicyBrowser({ policies }) {
  ...
  return (
    <div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:'12px', marginBottom:'1rem'}}>
        {policies.map(p => (
          <div key={p.policyID}
            ...
            style={{
              background: selected?.policyID === p.policyID ? '#EFF6FF' : '#fff',
              border: `1.5px solid ${selected?.policyID === p.policyID ? '#1652F0' : '#E2E8F0'}`,
              borderRadius:'10px', padding:'16px', cursor:'pointer', transition:'all 0.15s',
              boxShadow: selected?.policyID === p.policyID ? '0 4px 16px rgba(22,82,240,0.12)' : '0 1px 4px rgba(0,0,0,0.04)',
            }}>
            <div style={{fontSize:'18px', fontWeight:'700', color:'#1652F0', marginBottom:'6px'}}>{p.policyID}</div>
            <div style={{fontSize:'12px', color:'#475569', marginBottom:'8px', lineHeight:'1.4'}}>{p.policyName}</div>
            <div style={{display:'flex', gap:'6px', flexWrap:'wrap'}}>
              <span style={{background:'#F1F5F9', color:'#64748B', padding:'2px 6px', borderRadius:'4px', fontSize:'10px'}}>
                {p.energySlabs?.length} slabs
              </span>
              <span style={{background:'#F0FDF4', color:'#059669', padding:'2px 6px', borderRadius:'4px', fontSize:'10px'}}>
                ₹{Math.min(...(p.energySlabs?.map(s=>s.price)||[0]))}–₹{Math.max(...(p.energySlabs?.map(s=>s.price)||[0]))}/kWh
              </span>
            </div>
            <div style={{marginTop:'8px', fontSize:'10px', color:'#1652F0'}}>
      ...
      {/* Detail Panel */}
      {selected && (
        <div style={{background:'#F8FAFF', border:'1.5px solid #1652F0', borderRadius:'10px', padding:'20px', marginTop:'4px'}}>
          <div style={{display:'flex', justifyContent:'space-between', marginBottom:'16px'}}>
            <div>
              <div style={{fontSize:'18px', fontWeight:'700', color:'#1652F0'}}>{selected.policyID}</div>
              <div style={{fontSize:'13px', color:'#334155', marginTop:'3px'}}>{selected.policyName}</div>
              <div style={{fontSize:'11px', color:'#64748B', marginTop:'2px'}}>Program: {selected.programID}</div>
            </div>
          ...
          <div style={{marginBottom:'16px'}}>
            <div style={{fontSize:'11px', fontWeight:'700', color:'#94A3B8', letterSpacing:'1px', textTransform:'uppercase', marginBottom:'8px'}}>Energy Rate Slabs</div>
            {selected.energySlabs?.map((s, i) => {
              ...
              return (
                <div key={s.id} style={{display:'flex', alignItems:'center', gap:'12px',
                  background:'#F8FAFF', border:'1px solid #E2E8F0', borderRadius:'8px', padding:'10px 14px', marginBottom:'6px',
                  borderLeft:`3px solid ${colors[i%colors.length]}`}}>
                  <div style={{fontSize:'12px', color:'#64748B', flex:1}}>
                    <strong style={{color:'#1E293B'}}>{s.start}</strong> – <strong style={{color:'#1E293B'}}>{s.end ?? '∞'}</strong> kWh
                  </div>
                  <div style={{fontSize:'18px', fontWeight:'700', color:colors[i%colors.length]}}>
                    ₹{s.price}<span style={{fontSize:'11px', color:'#94A3B8'}}>/kWh</span>
                  </div>
                </div>
              )
            })}
          ...
          {selected.surchargeTariffs?.length > 0 && (
            <div>
              <div style={{fontSize:'11px', fontWeight:'700', color:'#94A3B8', letterSpacing:'1px', textTransform:'uppercase', marginBottom:'8px'}}>Surcharges / Time-of-Day</div>
              {selected.surchargeTariffs.map(s => (
                <div key={s.id} style={{display:'flex', alignItems:'center', gap:'12px',
                  background:'#F8FAFF', border:'1px solid #E2E8F0', borderRadius:'8px', padding:'10px 14px', marginBottom:'6px',
                  borderLeft:`3px solid ${s.value < 0 ? '#059669' : '#EA580C'}`}}>
                  <div style={{flex:1}}>
                    <div style={{fontSize:'12px', color:'#334155', fontWeight:'600'}}>{s.id}</div>
                  ...
                  <div style={{fontSize:'16px', fontWeight:'700', color: s.value < 0 ? '#059669' : '#EA580C'}}>
    ...
    <div>
      <div className="card">
        ...
        <div style={{display:'flex',gap:'0.75rem'}}>
          ...
          {(steps.length>0||result)&&!running &&
            <button className="btn btn-sm" style={{background:'#fff',border:'1.5px solid #E2E8F0',color:'#64748B'}}
        ...
        <div className="card">
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'0.6rem'}}>
            {STEP_INFO.map(info => {
              ...
              const color = st==='done'?'#059669':st==='running'?'#1652F0':st==='warn'?'#EA580C':'#CBD5E1'
              const iconBg = st==='done'?'#DCFCE7':st==='running'?'#DBEAFE':st==='warn'?'#FFF7ED':'#F1F5F9'
              return (
                <div key={info.id} style={{display:'flex',alignItems:'center',gap:'1rem',padding:'0.875rem 1rem',borderRadius:'10px',background:'#F8FAFF',borderLeft:`3px solid ${color}`,border:`1px solid #E2E8F0`,borderLeftWidth:3}}>
                  <div style={{width:'32px',height:'32px',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,background:iconBg,color,fontSize:'0.85rem',fontWeight:'800'}}>
                    {st==='done'?'✓':st==='running'?'…':info.id}
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:'0.875rem',fontWeight:'600',color:'#1E293B'}}>
                      Step {info.id}: {info.label} <span style={{fontSize:'0.72rem',color:'#94A3B8',fontWeight:'400'}}>{info.desc}</span>
                    </div>
                    {s?.detail && <div style={{fontSize:'0.775rem',color,marginTop:'0.2rem'}}>{s.detail}</div>}
                    {st==='running' && <div style={{fontSize:'0.75rem',color:'#1652F0',marginTop:'0.2rem'}}>Processing...</div>}
                  </div>
      ...
      {result && (
        <div className="card">
          ...
          <div style={{marginTop:'0.5rem',fontSize:'0.72rem',color:'#94A3B8',fontFamily:'monospace'}}>TXN: {result.txn}</div>
        </div>
      )}
    </div>
  )
    ...
    <div>
      <div className="card">
        ...
        <div style={{marginBottom:'0.875rem'}}>
          <div style={{fontSize:'0.72rem', color:'#64748B', marginBottom:'0.4rem', fontWeight:600}}>Consumer Type</div>
          <div style={{display:'flex', gap:'0.4rem', flexWrap:'wrap'}}>
            {['ALL','RESIDENTIAL','COMMERCIAL','INDUSTRIAL','AGRICULTURAL'].map(t => (
              <button key={t} onClick={() => { setConsumerFilter(t); setResult(null); setComparison(null) }}
                style={{
                  padding:'0.25rem 0.875rem', borderRadius:'999px', border:'1.5px solid',
                  fontSize:'0.72rem', cursor:'pointer', fontWeight:'600',
                  background:   consumerFilter === t ? '#1652F0' : '#fff',
                  borderColor:  consumerFilter === t ? '#1652F0' : '#E2E8F0',
                  color:        consumerFilter === t ? '#fff'    : '#64748B',
                  transition:   'all 0.15s',
                }}>
                {t}
              </button>
        ...
        {billingMonths > 1 && (
          <div style={{fontSize:'0.775rem', color:'#1652F0', marginBottom:'0.5rem', padding:'0.4rem 0.75rem', background:'#0d1a2d', borderRadius:'6px', display:'inline-block'}}>
        ...
        <div className="card">
          <div className="card-title">
            ...
            {result.billingMonths > 1 && (
              <span style={{fontSize:'0.78rem', color:'#64748B', fontWeight:'400', marginLeft:'0.75rem'}}>
          ...
          <div className="bill-box">
            ...
            <div className="bill-row" style={{fontWeight:'700', borderTop:'1px solid #30363d', paddingTop:'0.5rem'}}>
              ...
              <span className="amt" style={{color:'#059669', fontSize:'1rem'}}>₹{result.totalAmount?.toFixed(2)}</span>
            </div>
          </div>
          <div style={{marginTop:'0.75rem'}}>
            <div style={{fontSize:'0.75rem', color:'#64748B', marginBottom:'0.4rem'}}>Slab breakdown:</div>
                ...
                <tbody>
                  {result.slabBreakdown?.map(s => (
                    <tr key={s.slabId}>
                      ...
                      <td>{s.units} kWh</td>
                      <td>₹{s.rate}/kWh</td>
                      <td style={{color:'#059669'}}>₹{s.amount?.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {comparison && (
        <div className="card">
              ...
              <tbody>
                {comparison.map((c, i) => (
                  <tr key={c.policyId}>
                    ...
                    <td style={{fontSize:'0.8rem', color:'#334155'}}>{c.policyName?.slice(0,30)}</td>
                    <td style={{color:'#64748B'}}>₹{c.baseAmount?.toFixed(2)}</td>
          ...
          <div style={{marginTop:'0.875rem', fontSize:'0.75rem', color:'#64748B'}}>
  ...
  return (
    <div>
      <div className="card">
        ...
        {/* Slabs */}
        <div style={{fontSize:'0.75rem', color:'#64748B', marginBottom:'0.4rem', marginTop:'0.25rem'}}>Energy Slabs (Rs/kWh)</div>
            ...
            <tbody>
              {slabs.map((s,i)=>(
                <tr key={i}>
                  ...
                  <td>{slabs.length>1 && <button className="btn btn-sm" style={{background:'#3a1a1a',color:'#DC2626',border:'none'}} onClick={()=>rmSlab(i)}>×</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <button className="btn btn-sm" style={{background:'#F8FAFF',border:'1.5px solid #E2E8F0',color:'#64748B'}} onClick={addSlab}>+ Add Slab</button>

        {/* Surcharges */}
        {surcharges.length > 0 && (
          <div style={{marginTop:'1rem'}}>
            <div style={{fontSize:'0.75rem', color:'#64748B', marginBottom:'0.4rem'}}>Surcharges / ToD</div>
            {surcharges.map((s,i)=>(
              <div key={i} style={{display:'flex',gap:'0.5rem',flexWrap:'wrap',alignItems:'flex-end',background:'#F8FAFF',padding:'0.6rem',borderRadius:'6px',marginBottom:'0.4rem'}}>
                ...
                <button className="btn btn-sm" style={{background:'#3a1a1a',color:'#DC2626',border:'none'}} onClick={()=>rmSurcharge(i)}>Remove</button>
              </div>
            ))}
          </div>
        )}
        <button className="btn btn-sm" style={{background:'#F8FAFF',border:'1.5px solid #E2E8F0',color:'#64748B',marginTop:'0.5rem'}} onClick={addSurcharge}>+ Add Surcharge</button>

        <div style={{marginTop:'1rem',display:'flex',alignItems:'center',gap:'1rem'}}>
          <label className="checkbox-row">
            <input type="checkbox" checked={form.saveToFile} onChange={e=>upd('saveToFile',e.target.checked)} />
            <span style={{fontSize:'0.825rem',color:'#64748B'}}>Save & serve via BPP automatically</span>
          </label>
      ...
      {result && (
        <div className="card">
          ...
          <details>
            <summary style={{cursor:'pointer',color:'#1652F0',fontSize:'0.825rem'}}>View JSON</summary>
